...
fn spawn_gateway() -> Option<(std::process::Child, Mutex<std::process::ChildStdin>, BufReader<std::process::ChildStdout>)> {
...
fn init_gateway(stdin: &Mutex<std::process::ChildStdin>, reader: &mut impl BufRead) {
    let _ = send_request(stdin, reader, "initialize", json!({
...
fn test_concurrent_sandbox_operations() {
    ...
    for thread_id in 0..3 {
        ...
        std::thread::spawn(move || {
            ...
            for req_id in 0..3 {
                ...
                stdin_clone.lock().unwrap().write_all(line.as_bytes()).unwrap();
    ...
    let resp = send_request(&stdin_arc, &mut reader, "tools/call", json!({
        "name": "sandbox_list",
        "arguments": {}
    }));
...
fn test_malformed_json_request() {
    ...
    assert!(result.is_ok(), "Gateway should handle malformed input gracefully");
...
fn test_missing_required_fields() {
    ...
    let resp = send_request(&stdin, &mut reader, "tools/call", json!({
        "name": "sandbox_create",
        "arguments": {}
    }));
    ...
    assert!(has_error, "Should return error for missing template: {:?}", resp);
...
fn test_sandbox_long_timeout() {
    ...
    assert!(resp.get("result").is_some() || resp.get("error").is_some(),
        "Should handle long timeout: {:?}", resp);
...
fn test_rapid_sequential_requests() {
    ...
    for i in 0..10 {
        ...
        assert!(resp.get("result").is_some() || resp.get("error").is_some(),
            "Request {} should return response", i);
    }
...
fn test_empty_command_string() {
    ...
    // Create sandbox
    let create_resp = send_request(&stdin, &mut reader, "tools/call", json!({
        "name": "sandbox_create",
    ...
    if !sandbox_id.is_empty() {
        ...
        let handled = text.contains("error") || text.contains("empty") || resp.get("error").is_some();
...
fn test_very_long_command() {
    ...
    if !sandbox_id.is_empty() {
        ...
        assert!(resp.get("result").is_some() || resp.get("error").is_some(),
            "Should handle long command: {:?}", resp);
    }
...
fn test_special_characters_in_command() {
    ...
    if !sandbox_id.is_empty() {
        ...
        assert!(resp.get("result").is_some() || resp.get("error").is_some(),
            "Should handle special chars: {:?}", resp);
    }
...
fn test_unknown_tool_name() {
    ...
    // Should return error
    let has_error = resp.get("error").is_some()
        || extract_response_text(&resp).to_lowercase().contains("error");
    assert!(has_error, "Should return error for unknown tool: {:?}", resp);
...
fn test_invalid_jsonrpc_version() {
    ...
    assert!(result.is_ok(), "Gateway should handle invalid JSON-RPC version");
