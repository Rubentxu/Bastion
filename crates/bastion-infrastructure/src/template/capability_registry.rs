...
impl CapabilityRegistry {
        ...
        for entry in fs::read_dir(dir)? {
            ...
            match self.load_file(&path) {
                Ok(config) => {
                    ...
                    self.capabilities.write().unwrap().insert(name, config);
                    loaded += 1;
                }
                Err(e) => {
    ...
    pub fn contains(&self, capability: &str) -> bool {
        self.capabilities.read().unwrap().contains_key(capability)
    }
    ...
    pub fn get_config(&self, capability: &str) -> Option<CapabilityConfig> {
        self.capabilities.read().unwrap().get(capability).cloned()
    }
    ...
    pub fn reload(&self) -> Result<usize, CapabilityRegistryError> {
        ...
        let count = self.capabilities.read().unwrap().len();
        Ok(count)
    }
